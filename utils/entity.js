...
function prepareEntityForTemplates(entityWithConfig, generator) {
    ...
    generator.initPrimaryKey(entityWithConfig, entityWithConfig.databaseType);

    entityWithConfig.generateFakeData = type => {
        const fieldsToGenerate = entityWithConfig.fields.filter(field => !field.autoGenerated);
...
function preparePrimaryKeyIdsForTemplate(entityWithConfig, generator) {
    if (!entityWithConfig.embedded && entityWithConfig.primaryKey.ids === undefined) {
        entityWithConfig.primaryKey.ids = [
            ...
            ...entityWithConfig.relationships
                .filter(r => r.id && r.relationshipType === 'many-to-one')
                .flatMap(relationship => {
                    const otherEntity = generator.configOptions.sharedEntities[_.upperFirst(relationship.otherEntityName)];
                    preparePrimaryKeyIdsForTemplate(otherEntity, generator);
                    return otherEntity.primaryKey.ids.map(pk => {
                        return {
                            field: pk.field,
        ...
        entityWithConfig.primaryKey.ids.forEach(pk => {
...
module.exports = { prepareEntityForTemplates, loadRequiredConfigIntoEntity, preparePrimaryKeyIdsForTemplate };
